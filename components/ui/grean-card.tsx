...
const createDotPattern = (isDark: boolean) =>
...
const createWavePattern = (isDark: boolean) =>
  `data:image/svg+xml,${encodeURIComponent(`
...
const createGridPattern = (isDark: boolean) =>
  `data:image/svg+xml,${encodeURIComponent(`
...
const createRadialPattern = (isDark: boolean) =>
  `data:image/svg+xml,${encodeURIComponent(`
...
export const GreanCard: React.FC<GreanCardProps> = ({
  ...
  const getPattern = (patternType: string) => {
    switch (patternType) {
      case "dots":
        return createDotPattern(isDarkMode);
      case "waves":
        return createWavePattern(isDarkMode);
      case "grid":
        return createGridPattern(isDarkMode);
      case "radial":
        return createRadialPattern(isDarkMode);
      case "circles":
        return createRadialPattern(isDarkMode); // Use radial pattern for circles
      default:
        return "";
    }
  };
  ...
  return (
    ...
    >
      {pattern !== "none" && (
        <div
          ...
          style={{
            ...
            backgroundSize:
              pattern === "radial" || pattern === "circles"
                ? "80px 80px"
                : pattern === "waves"
                  ? "60px 60px"
                  : "60px 60px",
      ...
      {gradient && (
        <div
          className={`absolute inset-0 bg-gradient-to-br ${isDarkMode ? "from-[#3DD56D]/5 to-[#2bb757]/5" : "from-[#3DD56D]/8 to-[#2bb757]/8"}`}
        />
      )}
