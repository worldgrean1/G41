...
const SVG_PATTERNS = {
  ...
  waves: (
    ...
    >
      ...
      />
      <path
        d="M0,15 C25,18 75,8 100,15 L100,10 L0,10 Z"
        fill="#3dd56d"
        fillOpacity="0.08"
      />
    </pattern>
  ...
  geometric: (
    ...
    >
      <path
        d="M0,0 L30,15 L60,0 L45,30 L15,30 Z"
        fill="#4ade80"
        fillOpacity="0.1"
      />
  ...
  ({
    ...
    const cardStyles = useMemo(() => {
      const baseStyles = {
        ...
        borderColor: isDark
          ? "var(--grean-primary)/20"
          : "var(--grean-primary-light)/30",
        ...
        {!isLowPerformance && (
          <div className="absolute inset-0 opacity-20 pointer-events-none">
            <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
              <defs>{SVG_PATTERNS[patternType]}</defs>
              <rect
                width="100%"
                height="100%"
                fill={`url(#${patternType}-pattern)`}
              />
            </svg>
          </div>
        )}
