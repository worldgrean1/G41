...
const ProductImagePlaceholder = memo(
  ({ category, className = "" }: { category: string; className?: string }) => {
    const getIcon = () => {
    ...
    return (
      <div
        className={`flex items-center justify-center bg-gradient-to-br from-[#3dd56d]/20 to-[#4ade80]/20 rounded-xl ${className}`}
      >
        {getIcon()}
      </div>
    );
...
const ProductBadge = memo(
  ({ badge, sale }: { badge?: string; sale?: string }) => {
...
export const EnhancedProductCard = memo<EnhancedProductCardProps>(
  ({ product, onAddToCart, onViewDetails, className = "" }) => {
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="mt-auto">
              <div
                className={`rounded-xl p-4 mb-4 border-2 border-dashed transition-all duration-300 hover:border-solid ${
                  isDark
