  ...
  return (
    ...
    >
      <div className="space-y-4">
        {/* Title skeleton */}
        <div
          className={`h-6 rounded ${isDark ? "bg-slate-700" : "bg-gray-300"}`}
        />
        ...
        <div className="space-y-2">
          ...
          />
          <div
            className={`h-4 w-3/4 rounded ${isDark ? "bg-slate-700" : "bg-gray-300"}`}
          />
        </div>
        ...
        <div className="space-y-2 mt-6">
          {[1, 2, 3].map((i) => (
            <div key={i} className="flex items-center space-x-3">
              <div
                className={`w-4 h-4 rounded-full ${isDark ? "bg-slate-700" : "bg-gray-300"}`}
              />
              <div
                className={`h-4 flex-1 rounded ${isDark ? "bg-slate-700" : "bg-gray-300"}`}
              />
            </div>
          ))}
        </div>
        ...
        {/* Button skeleton */}
        <div
          className={`h-12 rounded-xl mt-auto ${isDark ? "bg-slate-700" : "bg-gray-300"}`}
        />
      </div>
    </div>
  );
...
export const IntegratedSolutionsGrid = memo<IntegratedSolutionsGridProps>(
  ({ solutions, onSolutionSelect, className = "", maxInitialCards = 4 }) => {
      ...
      <div className={`w-full ${className}`}>
        ...
        >
          ...
          <AnimatePresence>
            {loadingMore && (
              <>
                {Array.from({ length: solutions.length - maxInitialCards }).map(
                  (_, index) => (
                    <motion.div
                      key={`skeleton-${index}`}
