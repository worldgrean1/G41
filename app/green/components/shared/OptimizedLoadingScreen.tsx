...
export function OptimizedLoadingScreen({
  ...
  if (isLowPerformance || shouldReduceAnimations) {
    return (
      <div
        className={`fixed inset-0 z-50 flex items-center justify-center ${isDark ? "bg-gradient-to-br from-slate-900 to-slate-800" : "bg-gradient-to-br from-white to-white"}`}
      >
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-[#3DD56D]/30 border-t-[#3DD56D] rounded-full animate-spin mx-auto mb-4"></div>
          <h2
            className={`text-2xl font-bold ${isDark ? "text-white" : "text-black"} mb-2`}
          >
            Grean Energy
          </h2>
          ...
          <div
            className={`w-48 ${isDark ? "bg-slate-700" : "bg-gray-200"} rounded-full h-1 mb-2 mx-auto`}
          >
            <div
          ...
          <p
            className={`text-xs ${isDark ? "text-gray-400" : "text-black"} mb-3`}
          >
            {Math.round(progress)}%
          </p>
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center ${isDark ? "bg-gradient-to-br from-slate-900 to-slate-800" : "bg-gradient-to-br from-white to-white"} overflow-hidden`}
    >
      ...
      <div className="relative z-10 text-center max-w-md mx-auto px-6">
        ...
        {/* Title */}
        <h1
          className={`text-4xl font-bold ${isDark ? "text-white" : "text-black"} mb-2`}
        >
          Grean Energy
        </h1>
        ...
        {/* Progress bar */}
        <div
          className={`w-full ${isDark ? "bg-slate-700" : "bg-gray-200"} rounded-full h-2 mb-4 overflow-hidden`}
        >
          <div
        ...
        {/* Progress text */}
        <p
          className={`text-sm ${isDark ? "text-gray-400" : "text-black"} mb-4`}
        >
      ...
      {isComplete && (
        <div
          className={`absolute inset-0 ${isDark ? "bg-slate-900" : "bg-white"} animate-fade-out pointer-events-none`}
        />
      )}
